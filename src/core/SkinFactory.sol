...
contract SkinFactory is AccessControl {
    ...
    function deployCollection(address collectionAdmin) external onlyRole(FACTORY_ADMIN_ROLE) returns (address) {
    ...
    /// @notice
    function predictCreate2Address(bytes32 salt, address collectionAdmin) external view returns (address) {
        bytes memory bytecode = abi.encodePacked(type(SkinToken).creationCode, abi.encode(collectionAdmin));
